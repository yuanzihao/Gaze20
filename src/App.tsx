...
type OverlayAction = "complete" | "postpone" | "skip";
type OverlaySyncPayload = {
  remaining?: number;
  started?: boolean;
};
...
declare global {
  interface Window {
    __GAZE20_OVERLAY__?: OverlayConfig;
  }
}
...
function readOverlayConfig(): OverlayConfig | null {
  if (window.__GAZE20_OVERLAY__) return window.__GAZE20_OVERLAY__;

  const search = new URLSearchParams(window.location.search);
  if (search.get("overlay") === "1") {
    return {
      kind: search.get("kind") ?? "micro",
      seconds: search.get("seconds") ?? undefined,
      image: search.get("image") ?? undefined,
      score: search.get("score") ?? undefined
    };
  }
...
export default function App() {
  const [routeKey, setRouteKey] = useState(`${window.location.search}|${window.location.hash}`);

  useEffect(() => {
    const updateRoute = () => setRouteKey(`${window.location.search}|${window.location.hash}`);
  ...
  const overlayConfig = readOverlayConfig();
  return overlayConfig ? <OverlayWindow config={overlayConfig} key={routeKey} /> : <MainApp />;
}
...
// The overlay window is now a card-sized, OPAQUE, per-monitor popup (not a
// fullscreen transparent layer — that rendered invisible and blocked the desktop).
// So the card fills the whole window; the window itself IS the card.
...
const overlayCard: React.CSSProperties = {
  width: "100%",
  flex: 1,
  background: "linear-gradient(170deg,#dff0ea 0%,#edf6f2 40%,#f3faf7 100%)",
  overflow: "hidden",
  display: "flex",
  flexDirection: "column"
};
...
const overlayPill: React.CSSProperties = {
  ...
  gap: 7,
  background: "#fff",
  border: "1.5px solid #d8ebe3",
  borderRadius: 12,
  padding: "10px 18px"
};

const overlayBtnPrimary: React.CSSProperties = {
  display: "flex",
  alignItems: "center",
...
function OverlayWindow(props: { config: OverlayConfig }) {
  const kind = props.config.kind === "deep" ? "deep" : "micro";
  const total = clamp(Number(props.config.seconds ?? (kind === "micro" ? 20 : 180)), 5, 1800);
  const [remaining, setRemaining] = useState(total);
  const [startedAt, setStartedAt] = useState<number | null>(null);
  const sent = useRef(false);
  const started = startedAt !== null;
  const ringCirc = 2 * Math.PI * 64;
  const progress = started ? clamp(1 - remaining / Math.max(1, total), 0, 1) : 0;

  const sendAction = useCallback(async (action: OverlayAction) => {
    if (sent.current) return;
    sent.current = true;
    await safeInvoke("overlay_action", { action });
  }, []);

  // Start locally for instant feedback, and broadcast so the overlays on the
  // other monitors start their countdown at the same moment.
  const beginCountdown = useCallback(() => {
    setStartedAt((current) => current ?? Date.now());
    safeInvoke("overlay_start");
  }, []);
  ...
  // Sync: when any monitor starts the break, every overlay starts together.
  useEffect(() => {
  ...
  // Time-based countdown so every monitor stays in lockstep without tick drift.
  useEffect(() => {
  ...
  }, [remaining, sendAction, started]);

  const deepMinutes = Math.max(1, Math.round(total / 60));
  const countText = kind === "micro" ? String(remaining) : formatDuration(remaining);
  const title = kind === "micro" ? "该远眺一下了" : "该起身深休息了";
  const sub1 = kind === "micro" ? "专注了一会儿，先让眼睛远眺放松" : "你已连续高负荷用眼较久";
  const sub2 =
    kind === "micro"
      ? "看向 6 米外，保持 20 秒，并做 5–10 次完整眨眼"
      : `离开屏幕约 ${deepMinutes} 分钟，走动、补水、放松肩颈`;
      ...
      <div style={overlayCard}>
        <div style={{ position: "relative", height: 220, overflow: "hidden" }}>
          <svg
            ...
            fill="none"
          >
            <circle cx="360" cy="55" r="38" fill="#c8e4da" opacity="0.7" />
            <circle cx="60" cy="40" r="18" fill="#c8e4da" opacity="0.4" />
            <path d="M0 155 L60 100 L110 140 L170 88 L230 135 L290 90 L360 140 L420 100 L460 128 L460 220 L0 220 Z" fill="#b8d9ce" />
            <path d="M0 178 L70 130 L140 168 L210 118 L280 162 L360 122 L430 155 L460 140 L460 220 L0 220 Z" fill="#9dcbba" />
            <path d="M0 198 L90 164 L170 190 L250 158 L340 188 L460 170 L460 220 L0 220 Z" fill="#82b8a3" />
            <path d="M80 220 L90 188 L100 220 Z M200 220 L210 182 L220 220 Z M330 220 L340 192 L350 220 Z" fill="#6fa590" />
          </svg>
          <div
            style={{
              position: "absolute",
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <svg width="22" height="22" viewBox="0 0 50 50" fill="none">
                <ellipse cx="25" cy="25" rx="18" ry="11" fill="rgba(255,255,255,.9)" />
                <circle cx="25" cy="25" r="7" fill="#1f7a64" />
                <circle cx="25" cy="25" r="3" fill="#0c3d31" />
                <circle cx="22" cy="22" r="1.5" fill="#fff" />
              </svg>
              <span style={{ fontSize: 14, fontWeight: 700, color: "#1d3a32", opacity: 0.9 }}>远眺 Gaze20</span>
            </div>
          </div>
        </div>

        <div style={{ padding: "28px 36px 32px", textAlign: "center" }}>
          <div style={{ width: 170, height: 170, margin: "-55px auto 18px", position: "relative", zIndex: 3 }}>
            <svg width="170" height="170" viewBox="0 0 170 170">
              <circle cx="85" cy="85" r="74" fill="rgba(255,255,255,.72)" stroke="#d6ede5" strokeWidth="1" />
              <circle cx="85" cy="85" r="64" fill="none" stroke="#dceee8" strokeWidth="10" />
              <circle
                cx="85"
                cy="85"
                r="64"
                ...
                strokeWidth="10"
                strokeLinecap="round"
                strokeDasharray={`${(progress * ringCirc).toFixed(1)} ${ringCirc.toFixed(1)}`}
            ...
            >
              <div style={{ fontFamily: "'Manrope'", fontSize: kind === "micro" ? 62 : 44, fontWeight: 800, color: "#1f7a64", lineHeight: 1 }}>
                {countText}
              </div>
              {kind === "micro" && <div style={{ fontSize: 15, color: "#5f8074", fontWeight: 700, marginTop: 4 }}>秒</div>}
            </div>
          </div>

          <div style={{ fontSize: 28, fontWeight: 800, color: "#1d3a32", marginBottom: 10 }}>{title}</div>
          <div style={{ fontSize: 14.5, color: "#5f8074", lineHeight: 1.7, fontWeight: 500 }}>
            {sub1}
            <br />
            {sub2}
          </div>
          ...
          <div style={{ display: "flex", gap: 12, justifyContent: "center", marginTop: 20 }}>
            <div style={overlayPill}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#2caa7e" strokeWidth="2">
                <path d="M3 18 L9 9 L13 14 L21 5" />
              </svg>
              <span style={{ fontSize: 13.5, fontWeight: 700, color: "#41584f" }}>{kind === "micro" ? "远眺 20 秒" : "起身走动"}</span>
            </div>
            <div style={overlayPill}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#5b8def" strokeWidth="2">
                <path d="M2 12s4-6 10-6 10 6 10 6M9 15l-.5 2M15 15l.5 2" />
              </svg>
              <span style={{ fontSize: 13.5, fontWeight: 700, color: "#41584f" }}>{kind === "micro" ? "完整眨眼 5–10 次" : "补水 · 放松肩颈"}</span>
            </div>
          </div>

          <div style={{ display: "flex", gap: 10, marginTop: 22 }}>
            <button
              style={{ ...overlayBtnPrimary, flex: 1.4 }}
              onClick={() => (started ? sendAction("complete") : beginCountdown())}
            >
              {started ? (
            ...
            <button style={{ ...overlayBtnGhost, flex: 1 }} onClick={() => sendAction("postpone")}>
              延后 5 分钟
            </button>
            <button style={{ ...overlayBtnGhost, flex: 0.85 }} onClick={() => sendAction("skip")}>
              跳过
            </button>
          </div>

          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, marginTop: 16, fontSize: 12, color: "#90a59c" }}>
...
function ReminderDialog(props: {
  ...
  const progress = props.started ? clamp(1 - props.remaining / Math.max(1, props.total), 0, 1) : 0;
  const countText = props.kind === "micro" ? String(props.remaining) : formatDuration(props.remaining);
  const title = props.kind === "micro" ? "该远眺一下了" : "该起身深休息了";
  const sub =
    props.kind === "micro"
      ? "看向 6 米外，保持 20 秒，并做 5–10 次完整眨眼"
      : "离开屏幕，走动、补水、放松肩颈和眼睛";
    ...
    >
      <div
        style={{
          ...
          borderRadius: 28,
          overflow: "hidden",
          boxShadow: "0 40px 80px -24px rgba(10,40,30,.6),0 0 0 1px rgba(255,255,255,.3)"
        }}
      >
        <div style={{ position: "relative", height: 150, overflow: "hidden" }}>
          <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%" }} viewBox="0 0 420 150" preserveAspectRatio="xMidYMax slice" fill="none">
            <circle cx="330" cy="44" r="30" fill="#c8e4da" opacity="0.7" />
            <path d="M0 110 L70 70 L120 98 L180 60 L240 95 L300 62 L360 96 L420 70 L420 150 L0 150 Z" fill="#b8d9ce" />
            <path d="M0 126 L80 92 L150 118 L230 86 L310 116 L420 96 L420 150 L0 150 Z" fill="#9dcbba" />
          </svg>
          <div style={{ position: "absolute", top: 14, left: 0, right: 0, display: "flex", justifyContent: "center", alignItems: "center", gap: 8, zIndex: 2 }}>
            <svg width="20" height="20" viewBox="0 0 50 50" fill="none">
              <ellipse cx="25" cy="25" rx="18" ry="11" fill="rgba(255,255,255,.9)" />
              <circle cx="25" cy="25" r="7" fill="#1f7a64" />
              <circle cx="25" cy="25" r="3" fill="#0c3d31" />
            </svg>
            <span style={{ fontSize: 13, fontWeight: 700, color: "#1d3a32", opacity: 0.9 }}>远眺 Gaze20</span>
          </div>
        </div>
        <div style={{ padding: "0 30px 26px", textAlign: "center" }}>
          <div style={{ width: 140, height: 140, margin: "-48px auto 12px", position: "relative", zIndex: 3 }}>
            <svg width="140" height="140" viewBox="0 0 140 140">
              <circle cx="70" cy="70" r="62" fill="rgba(255,255,255,.8)" stroke="#d6ede5" strokeWidth="1" />
              <circle cx="70" cy="70" r="54" fill="none" stroke="#dceee8" strokeWidth="9" />
              <circle
                cx="70"
                cy="70"
                r="54"
            ...
            <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
              <div style={{ fontFamily: "'Manrope'", fontSize: props.kind === "micro" ? 48 : 34, fontWeight: 800, color: "#1f7a64", lineHeight: 1 }}>{countText}</div>
              {props.kind === "micro" && <div style={{ fontSize: 13, color: "#5f8074", fontWeight: 700, marginTop: 2 }}>秒</div>}
            </div>
          </div>
          <div style={{ fontSize: 23, fontWeight: 800, color: "#1d3a32", marginBottom: 8 }}>{title}</div>
          <div style={{ fontSize: 13.5, color: "#5f8074", lineHeight: 1.6, fontWeight: 500 }}>{sub}</div>
          <div style={{ fontSize: 11.5, color: "#90a59c", marginTop: 8 }}>软件内提醒 · 与各屏幕弹窗同步</div>
          <div style={{ display: "flex", gap: 10, marginTop: 18 }}>
            <button
              style={{ ...overlayBtnPrimary, flex: 1.3, padding: 13, fontSize: 14 }}
              onClick={props.started ? props.onComplete : props.onStart}
            >
              {props.started ? (
              ...
              {props.started ? "完成休息" : "开始休息"}
            </button>
            <button style={{ ...overlayBtnGhost, flex: 1, padding: 13, fontSize: 13.5 }} onClick={props.onPostpone}>延后 5 分钟</button>
            <button style={{ ...overlayBtnGhost, flex: 0.85, padding: 13, fontSize: 13.5 }} onClick={props.onSkip}>跳过</button>
          </div>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, marginTop: 13, fontSize: 11.5, color: "#90a59c" }}>
